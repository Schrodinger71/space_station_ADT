...
{
    /// <summary>
    ///     Entity system backing <see cref="ADTSubFloorHideCardComponent"/>.
    /// </summary>
    [UsedImplicitly]
    public abstract class SharedADTSubFloorHideCardSystem : EntitySystem
    {
        ...
        public override void Initialize()
        {
            ...
            SubscribeLocalEvent<ADTSubFloorHideCardComponent, ComponentStartup>(OnSubFloorStarted);
            SubscribeLocalEvent<ADTSubFloorHideCardComponent, ComponentShutdown>(OnSubFloorTerminating);
            // Like 80% sure this doesn't need to handle re-anchoring.
            SubscribeLocalEvent<ADTSubFloorHideCardComponent, AnchorStateChangedEvent>(HandleAnchorChanged);
            SubscribeLocalEvent<ADTSubFloorHideCardComponent, GettingInteractedWithAttemptEvent>(OnInteractionAttempt);
            SubscribeLocalEvent<ADTSubFloorHideCardComponent, GettingAttackedAttemptEvent>(OnAttackAttempt);
            SubscribeLocalEvent<ADTSubFloorHideCardComponent, GetExplosionResistanceEvent>(OnGetExplosionResistance);
        }

        private void OnGetExplosionResistance(EntityUid uid, ADTSubFloorHideCardComponent component, ref GetExplosionResistanceEvent args)
        {
            if (component.BlockInteractions && component.IsUnderCover)
                args.DamageCoefficient = 0;
        }

        private void OnAttackAttempt(EntityUid uid, ADTSubFloorHideCardComponent component, ref GettingAttackedAttemptEvent args)
        {
            if (component.BlockInteractions && component.IsUnderCover)
                args.Cancelled = true;
        }

        private void OnInteractionAttempt(EntityUid uid, ADTSubFloorHideCardComponent component, GettingInteractedWithAttemptEvent args)
        {
        ...
        private void OnSubFloorStarted(EntityUid uid, ADTSubFloorHideCardComponent component, ComponentStartup _)
        {
        ...
        private void OnSubFloorTerminating(EntityUid uid, ADTSubFloorHideCardComponent component, ComponentShutdown _)
        {
        ...
        private void HandleAnchorChanged(EntityUid uid, ADTSubFloorHideCardComponent component, ref AnchorStateChangedEvent args)
        {
            if (args.Anchored)
            {
        ...
        /// </summary>
        private void UpdateFloorCover(EntityUid uid, ADTSubFloorHideCardComponent? component = null, TransformComponent? xform = null)
        {
        ...
        private void UpdateTile(MapGridComponent grid, Vector2i position)
        {
            ...
            {
                if (!TryComp(uid, out ADTSubFloorHideCardComponent? hideComp))
                    continue;
        ...
        public void UpdateAppearance(
            EntityUid uid,
            ADTSubFloorHideCardComponent? hideComp = null,
